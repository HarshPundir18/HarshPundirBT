...
//MY Copy HTML files
gulp.task('templatesHTML:index', ['templatesHTML:views'], function() {
    log('Copying HTML PAGES files..3.9.1');
	return gulp.src(source.templatesHTML.index)
				.pipe(gulp.dest(build.templates.index));
});

gulp.task('templatesHTML:views', function() {
    log('Copying HTML VIEWS files.....3.9.1');
...
gulp.task('watch', function() {
	...
	gulp.watch(source.templatesHTML.views, ['templatesHTML:views']);         //JADE
	gulp.watch(source.templatesHTML.index, ['templatesHTML:index']);
